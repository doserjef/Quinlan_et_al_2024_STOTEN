# 3d-main-devel-apiary.R: script to run a JSDM using apiary density and 
#                         developed landcover as covariates in the model.
...
# Load initial values
load("results/inits-devel-apiary.rda")
inits.list$w <- NULL
inits.list$lambda <- NULL

# Run the model -----------------------------------------------------------
out <- lfMsAbund(formula = ~ scale(devel) + scale(apiary.density) + (1 | day.random) + 
...
save(out, file = paste0("/mnt/disk4/jeff/QDKG23/results/lfMsAbund-devel-apiary-chain-", 
                        chain, "-", Sys.Date(), ".R"))
