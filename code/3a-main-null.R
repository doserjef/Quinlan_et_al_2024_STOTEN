# 3a-main-null.R: script to run the null JSDM without either developed 
#                 landcover or apiary density as covariates in the model.
...
# Load initial values
load("results/inits-null.rda")
inits.list$w <- NULL
inits.list$lambda <- NULL

# Run the model -----------------------------------------------------------
out <- lfMsAbund(formula = ~ (1 | day.random) + factor(year) + 
                             factor(protocol) + effort:protocol,
                 data = data.list, 
                 n.batch = n.batch, 
...
save(out, file = paste0("/mnt/disk4/jeff/QDKG23/results/lfMsAbund-null-chain-", 
                        chain, "-", Sys.Date(), ".R"))
