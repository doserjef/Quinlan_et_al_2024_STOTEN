# 3b-main-developed.R: script to run a JSDM with developed landcover as a 
#                      covariate in the model using spAbundance
...
# Load initial values
load("results/inits-developed.rda")
inits.list$w <- NULL
inits.list$lambda <- NULL

# Run the model -----------------------------------------------------------
out <- lfMsAbund(formula = ~ scale(devel) + (1 | day.random) + 
...
save(out, file = paste0("/mnt/disk4/jeff/QDKG23/results/lfMsAbund-developed-chain-",
                       chain, "-", Sys.Date(), ".R"))
