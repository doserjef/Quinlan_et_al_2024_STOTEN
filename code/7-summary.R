# 7-summary.R: script to summarize all analysis results and generate all figures
...
# Generate figure of point locations --------------------------------------
...
# Figure S2
ggsave(file = 'figures/Figure-S2.png', units = 'in', width = 8, height = 4, bg = 'white')
...
# plot.order <- sp.names[order(beta.apiary.df$raw.counts)]
apiary.cat.plot <- beta.apiary.df %>%
  mutate(sp = factor(sp, levels = plot.order, ordered = TRUE)) %>%
    ...
    geom_segment(aes(x = low, y = sp, xend = high, yend = sp),
		 lineend = 'round', linewidth = 2, col = 'lightgray') +
...
fig.s3.plot <- plot.df %>%
    ggplot(aes(x = type, y = med)) +
      ...
      theme(text = element_text(family = 'LM Roman 10'), 
            ...
            plot.title = element_text(size = 17),
            legend.background = element_rect(fill = NA))
ggsave(plot = fig.s3.plot, file = 'figures/Figure-S3.png', device = 'png', 
...
# Generate plot as interpretable effect of apiary -------------------------
...
df <- data.frame(density = plot.apiary.density,
		 ...
		 X = coords.0[, 1],
		 Y = coords.0[, 2])
example.apiary.plot <- ggplot(df, aes(X, Y, fill = density))+ #variables
...
ggsave(file = 'figures/Figure-S1.png', units = 'in',
